    ...
    public function isAuth()
    {
        ...
        if ($this->access_token != '') {
            $check_valid_token = json_decode(
                $this->getCURL('https://api.vk.ru/method/users.get?v=5.103&access_token='
        ...
        if ($this->is_save) {
            ...
            if (file_exists($path)) {
                $cashed_data = json_decode(
                    base64_decode(str_replace($this->cashed_salt, '', @file_get_contents($path)), strict: true),
                    true,
                );
                if ($cashed_data != '') {
                    list($this->cookie, $this->access_token, $this->method, $this->scope, $this->id_app, $this->app) =
                        $cashed_data;
                }
            }
        }
