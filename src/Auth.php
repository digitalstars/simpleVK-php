...
class Auth
{
    ...
    public function isAuth(): int
    {
        ...
        if ($this->access_token != '') {
            $check_valid_token = json_decode(
                (string) $this->getCURL('https://api.vk.ru/method/users.get?v=5.103&access_token='
    ...
    private function generateAccessTokenApp(bool $resend = false): string
    {
        ...
        if (preg_match('!access_token=(.*?)&!s', (string) $access_token_location, $access_token) != 1)
            throw new SimpleVkException(
                0,
                'Не удалось найти access_token в строке ридеректа, ошибка:'
                . $this->getCURL($access_token_location, null, false)['body'],
            );
     ...
     */
    private function generateAccessTokenOfficialApp(
        string|false $captcha_key = false,
        string|false $captcha_sid = false,
    ): string {
        if (!isset($this->login) or !isset($this->pass))
            throw new SimpleVkException(
                0,
                'Для авторизации через оффициальное приложение необходимо задать логин и пароль',
            );
        ...
        $token_url =
            'https://oauth.vk.ru/token?grant_type=password'
            . '&client_id='
            . $this->app['id']
            . '&client_secret='
            . $this->app['secret']
        ...
        if (isset($response_auth['error']))
            throw new SimpleVkException(0, json_encode(
                $response_auth,
    ...
    private function saveCashed(): void
    {
        if ($this->is_save) {
            if (
                ...
                && !mkdir($concurrentDirectory = __DIR__ . '/cache')
        ...
        if ($this->is_save) {
            ...
            if (file_exists($path)) {
                $cashed_data = json_decode(
                    base64_decode(str_replace($this->cashed_salt, '', (string) @file_get_contents($path))),
                    true,
                );
                if (is_array($cashed_data)) {
                    list(
                        ...
                        $this->id_app,
                        $this->app,
                    ) = $cashed_data + [null, '', 1, DEFAULT_SCOPE, null, DEFAULT_APP['android']];
                }
            }
        }
    }
    ...
    private function loginInVK(): void
    {
        if (!isset($this->login) or !isset($this->pass))
            throw new SimpleVkException(
                0,
                'Для авторизации через приложение необходимо задать логин и пароль, либо куки',
            );
