...
trait FileUploader {
    ...
    private function sendFiles(string $url, string $local_file_path_or_url, string $type) {
        ...
        if (filter_var($local_file_path_or_url, FILTER_VALIDATE_URL) === false) {
            ...
            $post_fields = [$type => new CURLFile($tmp_filename, $mime_type, 'file.' . explode('/', (string)$mime_type)[1])];
        }
    ...
    private function uploadFile(string $upload_url, string $local_file_path_or_url, ?string $title, string $fileType, array $ex_params = []): string {
    ...
    public function getMsgAttachmentUploadDoc(int $peer_id, string $local_file_path_or_url, ?string $title = null): string {
