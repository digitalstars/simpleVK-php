...
// Проверки окружения при подключении библиотеки (composer files autoload).
...
if (PHP_VERSION_ID < 80200) {
    ...
    print
        Diagnostics::formatText(
            'SimpleVK3 требует PHP версии 8.2.0 или выше. Вы используете версию ' . PHP_VERSION,
            'red',
            need_dot: false,
        );
...
foreach ($required_extensions as $ext) {
    if (!extension_loaded($ext)) { // если хоть одного не хватает, вызываем диагностику модулей
        ...
        print
            Diagnostics::formatText(
                'SimpleVK не может работать, т.к. не установлены или не включены обязательные модули:',
                'red',
                need_dot: false,
            );
        Diagnostics::php_iniPatch();
