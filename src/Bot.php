...
class Bot {
     ...
     */
    public function btn(string|int $id, $btn = null, bool $is_callback = false, bool $is_text_triggered = false): MessageBot {
        $id = (string)$id;
        ...
        if (isset($btn)) {
            ...
            if (($this->config['btn'][$id][0] ?? null) == 'text' and ($is_text_triggered or $this->is_text_button_triggered))
            ...
            if (is_array($array)) {
                ...
                foreach ($array as $key => $row) {
                    ...
                    if (is_array($row)) {
                        ...
                        $out .= ($row) ? 'true' : 'false';
                    } elseif ((($stack[0] ?? null) == 'action') and ($key === 'func' or $key === 'func_after') and is_callable($row)) {
    ...
    /** Создает MessageBot вне контекста действия. */
    public function msg(?string $text = null): MessageBot {
        return MessageBot::create($this->vk, null, $this, $this->config['btn'])->text($text);
    }
    ...
    private function runAction(string|int $id, int|string $user_id, string|int $action_id, $result_parse = null, ?array $id_message = null, bool $is_edit = false) {
        $id = (string)$id;
...
class MessageBot extends Message {
    ...
    public static function create($vk = null, &$cfg = null, ?Bot $bot = null, &$buttons = null, ?string $id_action = null): static {
    ...
    public function eventAnswerOpenApp(int|string $app_id, int|string|null $owner_id = null, ?string $hash = null): static {
        $this->config['event'] = [
            'type' => 2,
            'app_id' => $app_id,
            'owner_id' => $owner_id,
     ...
     */
    public function edit(bool $is_save = true, array $save_params = ['text', 'img', 'doc', 'attachments', 'params', 'voice', 'kbd']): static|MessageBot {
