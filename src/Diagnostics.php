...
class Diagnostics
{
    ...
    private static function initialize()
    {
        ...
        print
            self::formatText(
                'Диагностика системы для работы с SimpleVK ' . SIMPLEVK_VERSION . " {$new_version_str}",
                'cyan',
                need_dot: false,
    ...
    public static function php_iniPatch()
    {
        ...
        if ($ini_file) {
            ...
            if (!str_ends_with($ini_file, '.ini')) {
                self::$final_text .= self::formatText(
                    'WARNING: Файл конфигурации не заканчивается на .ini . Возможно, у вас неправильно настроен интерпритатор в IDE или проблема в параметрах запуска',
                    'red',
                );
            }
        } else {
    ...
    private static function testFileOperations()
    {
        ...
        if (!mkdir($testDir, 0o755, true) && !is_dir($testDir)) {
    ...
    public static function checkModules()
    {
        self::$final_text .=
            self::EOL()
            . self::formatText('Проверка активации обязательных модулей в php.ini', 'cyan', need_dot: false);
        self::checkModuleGroup(['curl', 'json', 'mbstring']);
        self::$final_text .=
            self::EOL()
            . self::formatText('Проверка активации рекомендуемых модулей в php.ini', 'cyan', need_dot: false);

        self::checkModuleGroup(['ffi'], add_eol: false);
        self::$final_text .=
            self::formatText(
                ' (Используется в модуле на С для ускорения проверки и разбивки отправляемых сообщений)',
                'yellow',
                '',
                false,
            ) . self::EOL();
        ...
        self::$final_text .=
            self::formatText(
                ' (Используется для обнаружения дублирующихся событий от VK API и их игнорирования)',
                'yellow',
                '',
                false,
            ) . self::EOL();
        if (!self::isWindows()) {
            ...
            self::$final_text .=
                self::formatText(
                    ' (Автоматическая многопоточная обработка событий через Longpoll)',
                    'yellow',
                    '',
                    false,
                ) . self::EOL();
        ...
        self::$final_text .=
            self::EOL()
            . self::formatText(
                'Проверка активации других часто используемых модулей в php.ini',
                'cyan',
                need_dot: false,
            );
        self::checkModuleGroup([
            'mysqli',
            'pdo_mysql',
            'sqlite3',
            'pdo_sqlite',
    ...
    private static function getMemoryInfo()
    {
        ...
        if ($memtotal) {
            return self::formatText(
                'ОЗУ занято: ' . $memused . " / {$memtotal} GB (" . round(($memused / $memtotal) * 100) . '%)',
                'green',
            );
        }
    ...
    private static function getCpuInfo()
    {
        ...
        if (self::isWindows()) {
            ...
            if (!empty($output)) {
                ...
                $return_text .= self::formatText(
                    "Количество ядер: {$cpuCores}" . ($cpuFreq ? " ({$cpuFreq})" : ''),
                    'green',
                );
            ...
            $return_text .= $cpuName
                ? self::formatText("Процессор: {$cpuName}", 'green')
                : self::formatText('Не удалось получить информацию о названии процессора', 'yellow');
            $return_text .= $cpuCores
                ? self::formatText("Количество ядер: {$cpuCores}" . ($cpuFreq ? " ({$cpuFreq})" : ''), 'green')
                : self::formatText('Не удалось получить информацию о количестве ядер', 'yellow');
    ...
    public static function finish()
    {
        if (EnvironmentDetector::isWeb()) {
            ...
            self::$final_text .=
                self::EOL()
                . self::formatText('Не забудьте удалить этот скрипт после проверки!', 'yellow', need_dot: false);
        }
