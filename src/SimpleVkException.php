...
{
    /** Флаг, определяющий, нужно ли логировать ошибки в файл. */
    ...
    public static function logCustomError(string $message): void
    {
        if (self::$log_errors) {
            ...
            self::appendLog(self::formatCustomError($message));
        }
    }
    ...
    private static function appendLog(string $log_message): void
    {
        ...
        if (@file_put_contents($log_file, $log_message, FILE_APPEND | LOCK_EX) === false) {
            self::throwLogCreationError(
                custom_message: "По какой-то причине не удалось записать ошибку в файл лога по пути:\n{$log_file}",
            );
        }
    }
    ...
    private static function throwLogCreationError(
        ?string $object = null,
        ?string $path = null,
