...
final class DispatcherInternalsTest extends TestCase
{
    ...
    {
        $resolver = new \DigitalStars\SimpleVK\Dispatcher\ArgumentResolver();
        $config = ClientConfig::create('T', 1);

        return new Context(
            api: new \DigitalStars\SimpleVK\ApiClient($config, new FakeTransport()),
            dispatcher: new \DigitalStars\SimpleVK\Dispatcher\EventDispatcher(
                new \DigitalStars\SimpleVK\Dispatcher\DispatcherConfig([__DIR__ . '/tmp_actions']),
                $resolver,
            ),
    ...
    public function testContextGetSelfAndMissingFactory(): void
    {
        ...
        $noFactory = new \DigitalStars\SimpleVK\Dispatcher\Context(
            api: new \DigitalStars\SimpleVK\ApiClient(ClientConfig::create('T', 1), new FakeTransport()),
            dispatcher: $this->makeDispatcher(),
            argumentResolver: new \DigitalStars\SimpleVK\Dispatcher\ArgumentResolver(),
            event: [],
        );
    ...
    private function makeDispatcher(): \DigitalStars\SimpleVK\Dispatcher\EventDispatcher
    {
        return new \DigitalStars\SimpleVK\Dispatcher\EventDispatcher(
            new \DigitalStars\SimpleVK\Dispatcher\DispatcherConfig([__DIR__ . '/tmp_actions']),
            new \DigitalStars\SimpleVK\Dispatcher\ArgumentResolver(),
        );
    }
    ...
    public function testErrorReporterMiddlewareCatchesThrowable(): void
    {
        ...
        $reporter = new \DigitalStars\SimpleVK\Service\ErrorReporter($logger)->withTracePathFilter(
    ...
    public function testCallbackEventRoutingAndContextApi(): void
    {
        ...
        self::assertSame(1, \DigitalStars\SimpleVK\Tests\tmp_actions\PingBtn::$hits);
    ...
    public function testDebugMapContainsRoutes(): void
    {
        ...
        self::assertSame(
            \DigitalStars\SimpleVK\Tests\tmp_actions\StartAction::class,
            $debug['routes']['command']['/start'],
        );
        self::assertSame(
            \DigitalStars\SimpleVK\Tests\tmp_actions\PingBtn::class,
            $debug['routes']['payload']['PingBtn'],
        );
        self::assertSame(
            \DigitalStars\SimpleVK\Tests\tmp_actions\CatchAllAction::class,
